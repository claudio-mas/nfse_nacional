...
Branch: main
Repo: claudio-mas/nfse_nacional (local, sem remote)
Status: APPROVED
Mode: Builder
Supersedes: claudio-main-design-20260811-002626.md
Revision: 3 (sessão de resolução de gates — OQ1, OQ2, OQ3, OQ4, OQ6, OQ7 fechados com dado)
...
**Conseguir o certificado ICP-Brasil A1 e rodar o probe à mão, antes de escrever
`signing.py`.**

Não é uma tarefa de código. Compre ou pegue emprestado um A1, escreva trinta linhas de
script descartável que montam uma DPS mínima nos dois perfis, e mande as duas para
`https://sefin.producaorestrita.nfse.gov.br/SefinNacional/nfse`. Anote qual passou e qual
`E####` voltou.

O motivo: o design inteiro do Approach C se apoia na tese de que os dois pares merecem ser
testados porque ninguém sabe a resposta. Se o probe manual mostrar que só um funciona e o
outro devolve uma rejeição clara e estável, a feature de manchete encolhe para uma constante
e o v0.2.0 fica uma semana mais curto. Se mostrar que os dois passam, você acabou de
descobrir algo que nenhuma das duas bibliotecas concorrentes documentou — e isso vira o
primeiro post do README.

Das duas, uma economiza semanas de trabalho. A outra é a razão de a biblioteca existir.

## What I noticed about how you think

- Na revisão anterior você tinha escrito no seu próprio plano que *"a linha com valor `#Ref`
  na coluna de numeração da seção `RN DPS_NFS-e` é uma inconsistência no documento original
  do governo (fórmula Excel quebrada), não uma falha de conversão."* Esta sessão encontrou o
  terceiro caso do mesmo padrão: o ZIP de esquemas que o plano citava como `20260122` está
  publicado como `20260209`. Você tinha razão sobre a natureza da documentação — ela erra em
  metadados de forma sistemática — e a consequência prática é que datas e números vindos
  desses documentos precisam ser verificados na fonte, sempre.

- Você marcou com **ATENÇÃO** em maiúsculas o payload comprimido do `POST /nfse`, e citou
  *"as regras E1225 e E1226"* pelo número. A revisão 2 mostrou que essas regras estabelecem
  base64 e compressão sem nomear o algoritmo, e esta sessão confirmou que o algoritmo é gzip
  — que era o seu palpite original. Você marcou o ponto certo como o mais perigoso e acertou
  o valor; o que faltava era a evidência, não o instinto.

- No meio de uma spec de biblioteca fiscal você escreveu *"nunca logue o conteúdo da chave
  privada do certificado"* e *"thread-safe: não armazene estado mutável por requisição na
  instância"*. Ninguém coloca isso num plano inicial a não ser que já tenha visto dar errado.

- Nesta sessão, quando apresentei três rotas e a recomendada era a mais cara, você escolheu a
  mais cara sem negociar prazo. E na primeira decisão, tendo um design doc pronto de 679
  linhas na mão, você escolheu ir resolver as perguntas abertas em vez de começar a
  implementar. As duas escolhas são a mesma escolha: preferir descobrir a verdade agora a
  descobri-la em produção.
